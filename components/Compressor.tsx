...
export default function Compressor() {
  ...
  const [uses, setUsesState]       = useState(0)
  const [premium, setPremiumState] = useState(false)
  const inputRef                   = useRef<HTMLInputElement>(null)

  useEffect(() => {
    setUsesState(getUses())
  ...
  const handleFile = (f: File) => {
    ...
    if (f.size > 200 * 1024 * 1024) return setError('Máximo 200 MB por archivo.')
    setFile(f)
    setError('')
    setState('idle')
    setResult(null)
    ...
    <div className="w-full max-w-xl mx-auto">
      ...
      {state === 'idle' && (
        <>
          <div
            ...
            className={`relative border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer mb-6 transition-all duration-300 ${dragging ? 'border-brand bg-brand/5 shadow-[0_0_30px_rgba(226,75,74,0.3)]' : 'border-white/10 hover:border-brand/50 hover:shadow-[0_0_20px_rgba(226,75,74,0.15)]'}`}
          >
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['light', 'medium', 'max'] as Quality[]).map((q) => (
                <button key={q} onClick={() => setQuality(q)} className={`py-3 rounded-xl border text-sm font-semibold transition-all duration-200 ${quality === q ? 'border-brand text-brand bg-brand/10 shadow-[0_0_15px_rgba(226,75,74,0.25)]' : 'border-white/10 text-white/40 hover:border-white/20'}`}>
          ...
          <button onClick={handleCompress} disabled={!file} className="w-full py-4 rounded-xl bg-brand hover:bg-brand-dark disabled:bg-white/5 disabled:text-white/20 text-white font-bold text-lg transition-all active:scale-[0.99] hover:shadow-[0_0_25px_rgba(226,75,74,0.4)]">
            ⚡ Compress PDF
          </button>
      ...
      {state === 'optimized' && result && (
        <div className="text-center py-8">
          ...
          <p className="text-white/40 mb-2">This PDF is already fully optimized.<br />No compression possible without losing quality.</p>
          <p className="text-white/20 text-sm mb-8">{formatBytes(result.originalSize)} · optimized</p>
          <div className="bg-green-950/40 border border-green-800/30 rounded-xl p-4 mb-6">
            <p className="text-green-400 text-sm">💡 Try <span className="font-semibold">Max</span> compression level, or use a PDF with images for better results.</p>
          </div>
          <div className="flex gap-3">
            <button onClick={reset} className="flex-1 py-3 rounded-xl border border-white/10 text-white/50 hover:border-white/20 font-semibold transition-all">Try another PDF</button>
            <button onClick={() => downloadBlob(result.blob, result.filename)} className="flex-[2] py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-white/60 font-semibold transition-all">⬇ Download anyway</button>
          </div>
        </div>
      )}
      ...
      {state === 'done' && result && (
        <div className="mb-8">
          ...
          <div className="flex gap-3">
            <button onClick={reset} className="flex-1 py-3 rounded-xl border border-white/10 text-white/50 hover:border-white/20 font-semibold transition-all">New PDF</button>
            <button onClick={() => downloadBlob(result.blob, result.filename)} className="flex-[2] py-3 rounded-xl bg-brand hover:bg-brand-dark text-white font-bold transition-all active:scale-[0.99] shadow-[0_0_20px_rgba(226,75,74,0.3)] hover:shadow-[0_0_30px_rgba(226,75,74,0.5)]">⬇ Download</button>
          </div>
          {!premium && remaining <= 0 && (
            <p className="text-center text-white/30 text-sm mt-4">
      ...
      {state === 'paywall' && (
        <div className="bg-white/3 border border-white/10 rounded-2xl p-8 text-center shadow-[0_0_40px_rgba(226,75,74,0.1)]">
          ...
          <p className="text-white/40 mb-8">You've used your {FREE_LIMIT} free compressions.<br />One small payment, unlimited forever.</p>
          <div className="text-left space-y-3 mb-8">
            {['Unlimited compressions', 'All quality levels', 'Files up to 200 MB', 'No watermarks, ever'].map((perk) => (
          ...
          <button onClick={goToStripe} className="w-full py-4 rounded-xl bg-brand hover:bg-brand-dark text-white font-bold text-lg transition-all active:scale-[0.99] mb-3 shadow-[0_0_25px_rgba(226,75,74,0.4)] hover:shadow-[0_0_40px_rgba(226,75,74,0.6)]">
            Unlock Now — $1.99
          </button>
          <button onClick={() => setState('idle')} className="text-white/30 text-sm hover:text-white/50 transition-colors">Maybe later</button>
